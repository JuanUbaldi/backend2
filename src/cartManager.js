...
export default class CartManager {
  ...
  async addProductToCart(cartId, prodId, qty) {
    try {
      ...
      let prodInCart = carts[cartIndex].products.findIndex(
        (elem) => elem.id === prodId
      );
      if (prodInCart === -1) {
      ...
      await fs.promises.writeFile(this.path, JSON.stringify(carts, null, 2));
      return carts[cartIndex].products.length > 0
        ? carts[cartIndex].products[prodInCart]
        : "The cart is empty";
    } catch (err) {
      throw new Error(err);
    }
...
const asyncFn = async () => {
  ...
  /*   await cartsManager.addProductToCart(2, 2, 4);   */
};
asyncFn();
